...
调用：python scripts/stat_testcases.py 测试用例.xlsx 统计结果.xlsx`,
        ...
        version: 1,
        downloadCount: 0,
        references: [{ filename: '需求规格说明.md', content: `# 需求规格说明（测试用例生成参考节选）

> 文档标识：Q/ZL220701001-SRS
> 软件名称：智能计算仿真验证平台
...
#### 3.2.1.2 模型管理[US-YJJM-MXGL]

概述：模型管理提供模型的新增、删除、查询以及列表化显示模型的信息。

---

##### 3.2.1.2.2 新增模型[US-YJJM-MXGL-XZMX]
...
|------|------|
| 功能名称 | 新增模型 |
| 唯一标识 | US-YJJM-MXGL-XZMX |
| 适用阶段 | 软件开发、集成测试 |
| 优先级 | 高 |
| 可追溯性 | 智能计算仿真验证平台技术协议1.2 a) |
| 功能描述 | 提供模型文件在线新增功能，包括上传模型文件、添加模型基础信息。 |
| 输入 | 模型名称、程序类型、启动命令、停止命令、部署要求 |
| 输出 | 添加成功提示信息，模型列表自动刷新 |
| 处理过程 | 1. 提供新增模型表单页面；2. 填写模型基础信息，上传模型文件；3. 对模型信息进行校验，并进行保存入库操作；4. 返回模型新增成功/失败信息。 |
| 约束条件 | 模型名称不能重复 |
| 异常处理 | 模型若已存在，给出提示信息"模型已经存在或名称重复，请重新输入" |
| 页面导航 | 进入激励模型管理页面 |

**适用参数表**

| 序号 | 参数名称 | 数据类型 | 必填 | 输入方式 | 输入约束 | 精度 | 主键 |
|------|----------|----------|------|----------|----------|------|------|
| 1 | 模型名称 | 字符 | Y | 直接输入 | 最大字符数=255 | — | Y |
| 2 | 程序类型 | 字符 | Y | 直接输入 | C++、Python、Matlab等 | — | — |
| 3 | 启动命令 | 字符 | Y | 直接输入 | 最大字符数=2000 | — | — |
| 4 | 停止命令 | 字符 | Y | 直接输入 | 最大字符数=2000 | — | — |
| 5 | 部署要求 | 字符 | N | 直接输入 | 最大字符数=2000 | — | — |

---

### 3.2.1.3 模型组建[US-MXZJ]

概述：对硬件环境进行模型组建配置操作，通过图形化界面以拖拽方式将处理器、内存、存储、总线等元器件进行组装，形成对应的硬件数字模型。

---

##### 3.2.1.3.2 基础模型库[US-MXZJ-JCMXK]
...
|------|------|
| 功能名称 | 新增模型 |
| 唯一标识 | US-YJJM-MXGL-JCMXK |
| 适用阶段 | 软件开发、集成测试 |
| 优先级 | 高 |
| 可追溯性 | 智能计算仿真验证平台技术协议1.2 a) |
| 功能描述 | 基础模型库包括器件库和板卡库，其中器件库细分为CPU库、总线网络库、存储器库等，从模型管理中获取相关数据，并在界面中按类别进行显示。 |
| 输入 | 无 |
| 输出 | 模型ID、模型名称、模型类型、模型文件地址、模型配置文件地址 |
| 处理过程 | 1. 查询模型管理中相关模型数据；2. 将模型名称列表显示在界面中。 |
| 约束条件 | 无 |
| 异常处理 | 无 |
| 页面导航 | 进入组件模型页面 |
...
|------|------|
| 功能名称 | 模型组建 |
| 唯一标识 | US-YJJM-MXZJ-MXZJ |
| 适用阶段 | 软件开发、集成测试 |
| 优先级 | 高 |
| 可追溯性 | 智能计算仿真验证平台技术协议1.3 |
| 功能描述 | 用户通过拖拽方式从基础模型库拖拽出CPU模型、总线网络模型、存储器模型到画布中，点击模型组件进行参数配置，配置完所有模型参数后进行保存。 |
| 输入 | 组件ID、组件配置参数 |
| 输出 | 模型ID、模型名称、模型配置参数 |
| 处理过程 | 1. 从基础模型库选择CPU、总线、存储器组件拖拽到画布；2. 将画布中各组件进行连线；3. 对各模型组件进行参数配置；4. 将组建完成的模型进行保存。 |
| 约束条件 | 必须包含CPU组件 |
| 异常处理 | 无 |
| 页面导航 | 进入组件模型页面 |
...
| 序号 | 参数名称 | 数据类型 | 必填 | 输入方式 | 输入约束 | 精度 | 主键 |
|------|----------|----------|------|----------|----------|------|------|
| 1 | 模型ID | 字符 | Y | 自动生成 | 最大字符数=32 | — | — |
| 2 | 组件ID | 字符 | Y | 自动关联 | 最大字符数=32 | — | Y |
| 3 | 组件配置参数 | 字符 | Y | 直接输入 | JSON串，最大字符数=5000 | — | — |
| 4 | 模型名称 | 字符 | Y | 直接输入 | 最大字符数=50 | — | — |

---

##### 3.2.1.3.4.1 定时器模型[US-MXZJ-FT6713-DSQ]
...
|------|------|
| 功能名称 | FT6713定时器模型 |
| 唯一标识 | US-MXZJ-FT6713-DSQ |
| 适用阶段 | 软件开发、集成测试 |
| 优先级 | 高 |
| 可追溯性 | 智能计算仿真验证平台技术协议1.4.2 |
| 功能描述 | 在QEMU仿真平台中建立DSP FT-Q6713J/500处理器片上定时器（Timer）的功能建模。模型以系统总线设备形式集成，复现硬件定时器的计数、溢出及中断触发行为，用于虚拟系统中的任务调度、周期事件控制与外设同步。主要功能：1.周期计数与溢出建模 2.时钟源与分频控制 3.中断生成与响应机制 4.与DMA模块的协同机制 5.运行参数与控制配置 6.复位与初始化逻辑 7.系统集成与时间同步 |
| 输入 | 中断使能、复位信号、DMA请求使能 |
| 输出 | 中断标志位、DMA请求信号 |
| 处理过程 | （无描述） |
| 约束条件 | （无描述） |
| 异常处理 | 无 |
...
| 序号 | 接口名称 | 接口标识 | 提供方 | 调用方 |
|------|----------|----------|--------|--------|
| 1 | 模型部署接口 | ZL20241101002_IF_REQ_MODEL_DEPLOY | 智能计算仿真验证平台 | 客户仿真环境 |
| 2 | 模型启动接口 | ZL20241101002_IF_REQ_MODEL_STAR | 智能计算仿真验证平台 | 客户仿真环境 |
| 3 | 模型停止接口 | ZL20241101002_IF_REQ_MODEL_STOP | 智能计算仿真验证平台 | 客户仿真环境 |
| 4 | 全局变量监控接口 | ZL20241101002_IF_REQ_VARIABLE_MONITOR | 智能计算仿真验证平台 | 客户仿真环境 |

### 3.3.2 模型部署接口[ZL20241101002_IF_REQ_MODEL_DEPLOY]
...
|------|------|
| 接口名称 | 模型部署接口 |
| 标识 | ZL20241101002_IF_REQ_MODEL_DEPLOY |
| 接口描述 | 将模型文件同步下发到指定客户仿真环境的仿真节点中 |
| 调用方式 | http（Restful风格Post接口） |
| 接口优先级 | 高 |

**输入参数**

| 数据元素名称 | 唯一标识 | 数据类型 | 长度 | 是否可为空 | 描述 |
|-------------|----------|----------|------|------------|------|
| 模型ID | ModelID | 字符串 | 32 | 否 | 模型标识 |
| 模型文件ID | ModelFileId | 字符串 | 32 | 否 | 文件服务器中的模型文件ID |

**响应结果**

| 数据元素名称 | 唯一标识 | 数据类型 | 长度 | 描述 |
|-------------|----------|----------|------|------|
| 模型ID | ModelID | 字符串 | 32 | — |
| 模型文件路径 | ModelPath | 字符串 | 128 | 模型文件解压缩后的存储路径 |

\`\`\`json
{
  "statusCode": "200（成功）/ 500（失败）",
  "message": "提示语",
  "data": [{ "ModelID": "xxx", "ModelPath": "/usr/model/xxx" }]
}
\`\`\`
...
| 质量因素 | 要求描述 |
|----------|----------|
| 功能性 | 功能需求实现的完整性、正确性，功能实现覆盖率100% |
| 可靠性 | 具备接收异常输入不崩溃的能力；容错处理边界情况；对异常值丢弃；禁止浮点数相等比较；软件不低于24小时正常运行能力 |
| 易用性 | 按钮名称易懂，用词准确，用户无需查阅帮助即可完成操作 |
| 效率 | 本章节不适用 |
| 维护性 | 源程序注释率不低于20% |
| 可移植性 | 可在Windows 7操作系统下正常部署运行；目标仿真运行环境可在天脉操作系统下正常部署运行 |` }],
        scripts: [
          { filename: 'export_testcases.py', language: 'python', content: `#!/usr/bin/env python3
import sys
import json
import pandas as pd
...
COLUMNS = [
    "用例名称", "用例标识", "测试追踪", "测试追踪（技术协议）",
    "用例说明", "输入及操作说明", "测试数据", "期望测试结果",
    "前置条件", "测试类型", "设计人员", "设计日期",
    "执行人员", "监督人员", "监督日期", "执行日期", "测试结果"
]
...
def export(input_path, output_path):
    ...
    print(f"已生成: {output_path}，共 {len(df)} 条用例")

if __name__ == "__main__":
    export(sys.argv[1], sys.argv[2])` },
          { filename: 'stat_testcases.py', language: 'python', content: `#!/usr/bin/env python3
import sys
import pandas as pd

def stat(input_path, output_path):
    df = pd.read_excel(input_path)
    df = df.drop_duplicates(subset=["用例标识"])

    df["标识号"] = df["用例标识"].str.extract(r'^([^-]+-[^-]+)')

    rows = []
    for tag, group in df.groupby("标识号"):
        total = len(group)
        executed = group["测试结果"].notna().sum()
        not_executed = total - executed
        passed = (group["测试结果"] == "通过").sum()
        failed = executed - passed
        rows.append({
            "标识号": tag,
            "测试项名称": "",
            "测试用例数": total,
            "执行/未执行": f"{executed}/{not_executed}",
            "通过/未通过": f"{passed}/{failed}"
        })
